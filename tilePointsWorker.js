...
//
// Protocol: { id, cfg, tx, ty, qmData, imgData, landMask, imgRect } in →
...
self.onmessage = (e) => {
    const { id, cfg, tx, ty, qmData, imgData, landMask, imgRect } = e.data;
    try {
        const { positions, colors, count } = buildTilePoints(cfg, tx, ty, qmData, imgData, landMask, imgRect);
